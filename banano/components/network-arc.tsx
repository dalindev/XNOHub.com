...
import {
  StyleByAmount,
  getStyleByBananoAmount
} from '@/banano/lib/get-style-by-banano-amount';
...
const NetworkArcs: React.FC<NetworkArcsProps> = ({ nodes, earthRadius }) => {
  const { activeConfirmations } = useBananoConfirmations();
  const [arcs, setArcs] = useState<IArc[]>([]);

  const createArcStyles = (amount: number) => {
    const baseStyle = getStyleByBananoAmount(amount);
    return {
      broadcast: {
        ...baseStyle
      },
