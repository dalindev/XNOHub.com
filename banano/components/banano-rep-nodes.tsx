...
import NetworkArcs from '@/banano/components/network-arc';

interface BananoRepNodesProps {
  repsGeoInfo: IRepData[];
  earthRadius: number;
  onNodeHover: (nodeRepsGeoInfo: IRepData | null) => void;
}

const BananoRepNodes: React.FC<BananoRepNodesProps> = React.memo(
  ({ repsGeoInfo, earthRadius, onNodeHover }) => {
    ...
    const nodes = useMemo(() => {
      return repsGeoInfo.map((rep) => ({
        ...
        color: new THREE.Color(0xfbdc11)
      }));
    }, [repsGeoInfo]);
...
BananoRepNodes.displayName = 'BananoRepNodes';
    ...
    return (
      ...
      >
        <mesh geometry={barGeometry}>
          <meshBasicMaterial
            color={hovered ? 0x1a8223 : node.color}
